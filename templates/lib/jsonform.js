...
jsonform.elementTypes = {
  ...
  //hillj edits: modifying to display a previously loaded image if supplied
  //form should not overwrite the image unless a new image is uploaded.
  'file':{
    'template':'<span><% if(value){ %><img src="<%= escape(value) %>" /><% }%><input class="input-file" id="<%= id %>" name="<%= node.name %>" type="file" ' +
      '<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
      '/></span>',
    'fieldtemplate': true,
    'inputfield': true
